...
try
{
  ...
  $action = {
    ...
    # get string rep to alleviate errors in files with names containing spaces
    $source = "$source"
    
    # get extension of source
    $extension = [System.IO.Path]::GetExtension($source)
    # check if extension is in list of target extensions
    $isArchive = $extensions.Contains($extension)
    if ($isArchive) {
      # get destination root
      $destinationRoot = [System.IO.Path]::GetDirectoryName($source)
      # get destination dir name
      $destinationDir = [System.IO.Path]::GetFileNameWithoutExtension($source)
  ...
  Write-Host "Automatically extracting .7z, .zip, or .rar in $Path"
...
finally
{
  ...
  Write-Warning "Event Handler disabled, automatic extracting ended."
}